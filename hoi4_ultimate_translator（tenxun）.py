...
class HOI4UltimateTranslator:
    def __init__(self, secret_id, secret_key, source_lang="en", target_lang="zh", max_workers=4):
        """
        HOI4 Mod终极汉化工具 v3.0 (腾讯版)
    ...
    def generate_tc_signature(self, params):
        """生成腾讯云API签名"""
        # 1. 对参数排序
        sorted_params = sorted(params.items(), key=lambda x: x[0])
        sorted_params_str = urlencode(sorted_params)
        
        # 2. 构造待签名字符串
        sign_str = f"POST{self.endpoint}/?{sorted_params_str}"
    ...
    def _translate_text(self, text):
        ...
        try:
            ...
            # 生成签名
            signature = self.generate_tc_signature(params)
...
# ===== 用户配置区 =====
SECRET_ID = "111"  # 替换为你的腾讯云SecretId
SECRET_KEY = "111"  # 替换为你的腾讯云SecretKey
MOD_DIRECTORY = r"E:\gtxx4\localisation"  # 替换为mod本地化文件夹路径
MAX_WORKERS = 2  # 腾讯API并发限制严格，建议2个线程
# ======================

if __name__ == "__main__":
    print("=" * 70)
    print("HOI4 Mod终极汉化工具 v3.0 (腾讯版)")
    print("=" * 70)
    
    # 配置检查
       
    # 配置检查
    if SECRET_ID == "your_secret_id" or SECRET_KEY == "your_secret_key" or MOD_DIRECTORY == "FULL_PATH_TO_YOUR_MOD_LOCALIZATION":
        print("配置错误: 请先修改脚本中的配置信息!")
        print("1. 打开本脚本文件")
        print("2. 修改顶部的SECRET_ID, SECRET_KEY和MOD_DIRECTORY")
        print("3. 保存后重新运行")
        input("\n按Enter键退出...")
        sys.exit()
