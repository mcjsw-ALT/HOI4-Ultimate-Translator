...
class HOI4UltimateTranslator:
    def __init__(self, app_id, api_key, api_secret, source_lang="en", target_lang="zh", max_workers=4):
        """
        HOI4 Mod终极汉化工具 v3.0 (讯飞版)
        完美解决格式问题、变量引用、专有名词保护和引用链解析
        """
        self.app_id = app_id
        ...
        self.endpoint = "https://itrans.xfyun.cn/v2/its"
        
        # 智能正则表达式系统
        self.text_regex = re.compile(r'^(\s*[^\s:]+(?::\d+)?\s+)(".*?")$', re.MULTILINE)
        self.var_regex = re.compile(r'(\$[^\s]+\$|§[^\s]+\$|§.)')
    ...
    def generate_xunfei_signature(self):
        """生成讯飞API所需的签名"""
        # 生成RFC1123格式的日期
        date = datetime.utcnow().strftime('%a, %d %b %Y %H:%M:%S GMT')
        
        # 构造签名字符串
        signature_origin = f"host: itrans.xfyun.cn\ndate: {date}\nGET /v2/its HTTP/1.1"
        ...
        # Base64编码
        signature_b64 = base64.b64encode(signature).decode(encoding='utf-8')
    ...
    def load_protected_terms(self):
        """加载专有名词保护列表"""
        default_terms = ["Kiel", "Panama", "Dam", "NATO", "Warsaw", "Soviet", "Allies", "Axis"]
        try:
    ...
    def _translate_text(self, text):
        ...
        try:
            # 生成签名和日期
            authorization, date = self.generate_xunfei_signature()
            ...
            data = {
                ...
                "payload": {
                    "text": base64.b64encode(protected_text.encode('utf-8')).decode('utf-8')
                }
            }
            ...
            if result["header"]["code"] != 0:
                ...
                print(f"\n讯飞翻译错误 (代码 {result['header']['code']}): {error_msg}")
                self.error_count += 1
                return original_text
            
            # 解码翻译结果
            translated_base64 = result["payload"]["result"]["text"]
            translated_text = base64.b64decode(translated_base64).decode('utf-8')
    ...
    def _process_references(self, content):
        ...
        for ref in ref_matches:
            ...
            if ref_key in self.global_translation_map:
                ...
                content = content.replace(ref, translated_value)
    ...
    def process_directory(self, directory_path):
        ...
        # 按优先级排序
        yaml_files.sort(key=lambda x: x)
        sorted_files = [f[1        sorted_files = [f for f in yaml_files]
...
# ===== 用户配置区 =====
# 讯飞翻译API配置
APP_ID = "111"  # 替换为你的讯飞APP ID
API_KEY = "111"  # 替换为你的讯飞API Key
API_SECRET = "111"  # 替换为你的讯飞API Secret
MOD_DIRECTORY = r"E:\gtxx4\localisation"  # 替换为mod本地化文件夹路径
MAX_WORKERS = 2 # 并发线程数，根据电脑性能调整(2-4)
# ======================
...
if __name__ == "__main__":
    print("=" * 70)
    print("HOI4 Mod终极汉化工具 v3.0 (讯飞版)")
    print("=" * 70)
    
    # 配置检查
    if APP_ID == "your_app_id" or API_KEY == "your_api_key" or API_SECRET == "your_api_secret" or MOD_DIRECTORY == "FULL_PATH_TO_YOUR_MOD_LOCALIZATION":
        print("配置错误: 请先修改脚本中的配置信息!")
        print("1. 打开本脚本文件")
        print("2. 修改顶部的APP_ID, API_KEY, API_SECRET和MOD_DIRECTORY")
        print("3. 保存后重新运行")
        input("\n按Enter键退出...")
        sys.exit()
